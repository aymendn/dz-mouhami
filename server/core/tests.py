...
class IsLoggedInTest(TestCase):
    """
    test if user is logged in in any view that requires login,
    ...
    def setUp(self): 
        ...
        profile = LawyerProfile.objects.create(user=user, address=address, bio="this is a test bio")
    ...
    def test_login_success(self):
        ...
        self.assertEqual(response_dict["bio"],  "this is a test bio") # assert that the response contains the user's bio
...
class AppointmentTest(TestCase):
    ...
    def setUp(self):
        ...
        self.token = Token.objects.create(user=self.user, 
                                          key="89aef6b7d25c46h7859f41352q595teec6012708")
        self.address = Address.objects.create(id=1, city="test city", street="test street")
        self.lawyer = LawyerProfile.objects.create(user=self.user, address=self.address, bio="this is a test bio")
        # add a client
        self.user2 = User.objects.create(id=2, username='testclient', password='123456')
        self.client = ClientProfile.objects.create(user=self.user2, address=self.address, age=19)
        # add an appointment
        self.timeslot = TimeSlot.objects.create(id=1, start_time="10:00:00", end_time="11:00:00", 
                                                day="2020-05-01", lawyer=self.lawyer)
        self.appointment = Appointment.objects.create(id=1, lawyer=self.lawyer, client=self.client, 
                                                      date="2020-05-01", time_slot=self.timeslot, 
                                                      status="Pending")
        # setup the client and the url
        self.client = Client()
    ...
    def test_accept_appointment(self):
        ...
        self.assertEqual(response.content, b'{"success":true,"message":"Appointment accepted."}')
